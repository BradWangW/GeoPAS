...
PROTOCOLS=("lio" "random" "lpo")                # "lpo", "lio", or "random"
								...
								for cat_loss_weight in "${CAT_LOSS_WEIGHT_LIST[@]}"; do
									for cat_tau in "${CAT_TAU_LIST[@]}"; do
										...
										if [[ "$tail_penalty" == "1" ]]; then
											...
											run_out_dir="${OUT_DIR_BASE}/scale${target_scale}_tail${tail_penalty}_cap${tail_lam_cap}_thr${tail_lam_thr}_dual${dual_head}_w${cat_loss_weight}_tau${cat_tau}_pen${cat_penalty}"
										mkdir -p "$run_out_dir"

											echo "Starting: target_scale=$target_scale tail_penalty=$tail_penalty tail_lam_cap=$tail_lam_cap tail_lam_thr=$tail_lam_thr dual_head=$dual_head cat_loss_weight=$cat_loss_weight cat_tau=$cat_tau cat_penalty=$cat_penalty"
